...
### Step 5: Clean Old Loops

Scan the loop ledger directory (`lfgLoop.loopsDir`, default `.athanor/loops/`):
- For each `.athanor/loops/<loop_id>/`, read `state.json` (`cycle_state`) and
  `loop.md` (the `status:` field).
- A loop is a **cleanup candidate** only when BOTH hold:
  1. **Non-completing terminal status** — `loop.md status == abandoned`, or
     ...
     `skills/lfg-loop/references/state-shape.md`).
  2. **Aged past retention** — older than `lfgLoop.loopRetentionDays` (default
     30), measured from the most recent receipt / `state.json` timestamp, or
     the loop directory mtime when no timestamp is recorded.
- For each candidate:
  - Promote any `<!-- importance: permanent -->` discoveries/receipts to
    `.athanor/lessons/` first (mirror Step 1).
  - Then delete the `.athanor/loops/<loop_id>/` directory.

**NEVER** clean a loop whose `status == complete` — completed loops are archived
to `docs/loops-completed/<loop_id>/` by the lfg-loop completion path, and
deleting the live tree of a completed loop requires explicit user action
(athanor convention). **NEVER** clean an `active` / in-progress loop, and
**NEVER** clean a loop younger than `loopRetentionDays`.
...
Loops:
  - Total: {count}
  - Cleaned: {count} (non-completing, older than {loopRetentionDays} days)
...
6. **NEVER** clean a `complete` loop (archived to `docs/loops-completed/`; deleting its live tree is a user action) or an `active` loop
