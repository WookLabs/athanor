#!/usr/bin/env python3
"""CLI for Athanor durable lfg-loop loop decisions."""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Decide the next action for an Athanor durable lfg-loop loop."
    )
    ...
    try:
        ...
        if args.trace_path is not None:
            _write_trace(
                ...
                trace_id=args.trace_id or f"loop-{decision.loop_id}",
