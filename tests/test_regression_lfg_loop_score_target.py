"""Regression: lfg-loop score-target optimization loop contract."""
...
LFG_GOAL_SKILL = REPO_ROOT / "skills" / "lfg-loop" / "SKILL.md"
JUDGE_RUBRIC = REPO_ROOT / "skills" / "lfg-loop" / "references" / "judge-rubric.md"
GOAL_TEMPLATE = REPO_ROOT / "skills" / "lfg-loop" / "references" / "loop-md-template.md"
CODEX_LFG_GOAL = (
    REPO_ROOT / "plugins" / "athanor-codex" / "skills" / "athanor-lfg-loop" / "SKILL.md"
)
...
def test_lfg_loop_documents_score_target_loop() -> None:
    ...
    assert not missing, f"lfg-loop SKILL.md missing score-target contract: {missing}"
...
def test_loop_template_supports_optional_score_target_section() -> None:
...
def test_score_target_config_defaults_and_schema() -> None:
    ...
    assert root["lfgLoop"]["scoreTarget"] == expected
    assert template["lfgLoop"]["scoreTarget"] == expected

    props = schema["properties"]["lfgLoop"]["properties"]["scoreTarget"]["properties"]
    for key in expected:
        assert key in props
