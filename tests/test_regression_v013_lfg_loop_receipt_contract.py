...
def test_valid_receipt_fixture_exists():
    """MUST: ``tests/fixtures/lfg_loop/receipt_valid.md`` exists.
...
def test_invalid_missing_step3_fixture_exists():
    """MUST: ``tests/fixtures/lfg_loop/receipt_invalid_missing_step3.md`` exists.
...
def test_partial_with_residuals_fixture_exists():
    """MUST: ``tests/fixtures/lfg_loop/receipt_partial_with_residuals.md`` exists.
...
def test_receipt_validator_has_9_step_verification_table():
    """MUST: ``skills/lfg-loop/references/receipt-validator.md`` declares the 9-step verification table.
    ...
    1. The validator dispatch prompt file exists at
       ``skills/lfg-loop/references/receipt-validator.md``. (Subtask 7
       ...
       in Agent() dispatch by the lfg-loop skill leader, so the
...
# - loop-md-template.md (canonical loop.md structure)
#
...
def test_judge_rubric_exists_and_has_rubric_axes():
    p = REPO_ROOT / "skills" / "lfg-loop" / "references" / "judge-rubric.md"
    assert p.is_file()
...
def test_state_shape_has_cycle_state_enum():
    p = REPO_ROOT / "skills" / "lfg-loop" / "references" / "state-shape.md"
    assert p.is_file()
    body = p.read_text(encoding="utf-8")
