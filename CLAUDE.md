...
- In `/athanor:lfg` / `/athanor:lfg-loop`, under that pipeline's explicit user authorization, the leader may run git/gh plumbing (commit, push, open PR, CI watch, and — opt-out (default ON; `--unmerge` to disable) — merge a green PR: merge only, never version-bump/tag/CHANGELOG/STATE.md; that stays the releaser ceremony) and read session/repo files for verification.
...
| `/athanor:lfg-loop` | Execute | **Loop-engineering macro harness** — accepts a natural-language objective and runs research/discovery, planning, architecture/design, LFG implementation cycles, assessment/review, verification, persistence, and next-loop decisioning. |
...
| Scope Drift Detection | **on-demand** — `skills/scope-drift/SKILL.md` user-invoked only; no auto-fire by default |
...
4. **Explicit evidence gates.** `/athanor:lfg-loop`, `/athanor:lfg`, and `/athanor:work` carry quality through receipts, work logs, assessment/review artifacts, controller decisions, and user ratification where required.
