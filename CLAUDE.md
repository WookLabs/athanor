...
- In `/athanor:lfg` / `/athanor:lfg-goal`, under that pipeline's explicit user authorization, the leader may run git/gh plumbing (commit, push, open PR, CI watch, and — opt-in (default off) — merge a green PR: merge only, never version-bump/tag/CHANGELOG/STATE.md; that stays the releaser ceremony) and read session/repo files for verification.
