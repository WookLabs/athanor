...
- In `/athanor:lfg` / `/athanor:lfg-goal`, under that pipeline's explicit user authorization, the leader may run git/gh plumbing (commit, push, open PR, CI watch, and — opt-out (default ON; `--unmerge` to disable) — merge a green PR: merge only, never version-bump/tag/CHANGELOG/STATE.md; that stays the releaser ceremony) and read session/repo files for verification.
...
### Athanor-native (13 user-invocable + 2 internal: scope-drift, verification-before-completion)
...
| `/athanor:plan` | Plan | Canonical cross-model planning via `--depth={standard\|deep\|lite}` (+ `--no-review`). Standard = Planner A + review + Critic; deep = dual planners + synthesis Critic; lite = Planner A only. `/athanor:deep-plan` and `/athanor:lite-plan` are thin wrappers. |
| `/athanor:deep-plan` | Plan | Thin wrapper for `/athanor:plan --depth=deep`; rejects contradictory `--depth=standard\|lite`; normal `plan.md` artifacts only. |
| `/athanor:lite-plan` | Plan | Thin wrapper for `/athanor:plan --depth=lite`; rejects contradictory `--depth=standard\|deep`; normal `plan.md` artifacts only. |
...
5. 작업 완료 시 lessons를 2-tier(permanent/working)로 추출·저장한다 (`.athanor/lessons/` frontmatter `importance`; 로컬 read-only recall은 `docs/memory-index.md`, goal/session handoff는 `docs/handoff-artifact.md`; mem-search 영구저장은 미구현 — STATE.md Known gaps)
...
6. **Skill responsibilities:**
   - `/athanor:plan`, `/athanor:discuss`, `/athanor:prompt-gen`: reuse `<LATEST>` if it has no `work-log.md`; else create a new session.
...
|---|---|
| Completion-Claim Verification (Stop hook) | **enforced (command-based)** — `hooks/hooks.json` registers a `type: command` Stop hook invoking `scripts/hooks/stop_verify_claims.py`. Detects material claims via v0.7.7 English+Korean whitelist + v0.10.2 NFKC unicode normalization + Cyrillic/Greek/Armenian confusables fold + paraphrase regex + v0.10.2 vendor-aware whitelist (CE/superpowers idioms) + v0.10.3 conditional-tense + attribution suppression. Exits 2 to block Stop with stderr fed back as continuation context; emission sentinel `<!-- athanor:verification-emission v=2 nonce=... -->` prevents re-entry. `athanor.json` `hooks.profile: "off"` opt-out. **v0.10.0 scope** — vendor-aware whitelist applies to vendored CE/superpowers prose idioms (v0.10.1+ refinement). v0.11.3 input-layer fix + v0.11.4 plugin-root deployment fix close the companion-fix arc. The hook also blocks a material claim paired with a **terminal `VERDICT: FAIL`/`PARTIAL`** (`_terminal_verdict`, enforced/command-based, same class) — catches the claim-contradicts-its-own-evidence case; a terminal `VERDICT: PASS` is NOT a release path (the v=2 sentinel stays the sole exit-0), and silent VERDICT omission still falls back to the whitelist gate. Spike evidence: `docs/STATE.md` §"Command-hook Stop blocking spike (2026-05-18)". See `docs/archive/stop-hook-postmortem.md` and `docs/archive/defense-mechanisms-detail.md` for historical detail. **v0.24.3 portable invocation:** the hook is invoked via `sh scripts/hooks/run_hook.sh` (functionality-probed `python3` → `python` → `py -3`, ≥3.10 — the Windows Store `python3` stub fails the probe and is skipped); hosts with NO working interpreter degrade to a **visible no-op** (exit-1 hook error + stderr notice, never a brick) — documented fail-open residual closing the 2026-07-01 silent-fail-open incident class. |
...
| using-superpowers boundary (v0.11.1) | **advisory (preamble-declared)** — `superpowers:using-superpowers` is a Claude Code platform mechanism surfaced in SessionStart context, not Athanor hooks. In the **13 native Thin Leader skills** (assess, analyze, debug, deep-plan, discuss, lfg, lfg-goal, lite-plan, plan, prompt-gen, review, setup, work), that pre-response pressure is **advisory here**: discovery resolves through leader dispatch, under planner-classified discipline. Each skill points to the canonical declaration below. |
| PreToolUse Kernel Guard | **enforced (command-based), best-effort coverage** — `hooks/hooks.json` PreToolUse event; the hook genuinely fires and exits 2 to block. Targets 3 accident-class patterns: destructive shell (rm -rf /, git reset --hard), force-push to main/master, credential file access (.env, private keys). `.env.example`/`.env.test` allowed. `hooks.profile: "off"` opt-out. **Honest scope:** this is a textual regex guard, NOT a command parser or security boundary — it catches obvious literal forms but is bypassable by obfuscation (command substitution `$(...)`, variable indirection, base64/eval, reordered flags). Treat it as a guardrail against fat-finger accidents, not containment against an adversary. The force-push matcher uses a `(?![\w-])` boundary so `main`/`master`-prefixed branches (e.g. `feature/main-update`) are allowed while exact `main`/`master` segments stay blocked. **v0.24.3 portable invocation:** invoked via `sh scripts/hooks/run_hook.sh` (same functionality-probed interpreter resolution as the Stop hook); on python-less hosts the guard fails OPEN with a visible exit-1 hook error — consistent with its documented fat-finger-guardrail scope, not a security boundary. |
| lfg merge-readiness gate (Step 8.5) | **advisory (leader-prose-enforced)** — `/athanor:lfg` Step 8.5 opt-out auto-merge (default ON; `--unmerge` to disable; `--merge` to force-enable a `false` config → `athanor.json` `lfg.autoMerge`) runs a fail-loud conjunctive readiness gate (G0–G5: re-entry/draft/dual-source review blockers/unresolved-CI/exhaustive 8-value `mergeStateStatus` disposition/merge-queue) before `gh pr merge --rebase`. Same enforcement class as the leader-prose Step 5/7/8 lfg plumbing — there is **no PreToolUse/Stop runtime hook** that programmatically blocks the merge or enforces the conjunction (same leader-bound class as the Step 3/8 fix-round counter: **advisory (leader-bound exit-code)** via `scripts/loops/lfg_fix_round_counter.py bump --loop review|ci` exit-3-at-cap, a strict upgrade over prose, but no runtime hook forces the branch). Gate-fail leaves the PR open, names the failing clause, and still emits DONE; it never `--admin`-bypasses protection and never version-bumps/tags/CHANGELOG (that stays the releaser ceremony). The G0–G4 disposition is now computed in code by `scripts/gates/lfg_merge_gate.py` (verdict-only pure function; unknown `mergeStateStatus`/`mergeable` enum → fail-loud exit 2) — a strict upgrade over hand-interpreted bash, but still leader-invoked (no runtime hook forces the verdict, so it stays **advisory**). `tests/test_regression_lfg_merge_gate.py` locks the prose logic; `tests/test_lfg_merge_gate_exec.py` locks the executable verdict. **NOT enforced** — do not read this row as a runtime gate. |

### Completion-Claim Verification (Stop hook — enforced, command-based)
On every `Stop` event, Claude Code invokes `scripts/hooks/stop_verify_claims.py` (registered as `type: command` in `hooks/hooks.json`). The script extracts the last main-session assistant message, checks the `hooks.profile: "off"` opt-out, recognises its own current `<!-- athanor:verification-emission v=2 nonce=<32-hex> -->` sentinel (re-entry prevention), greps for material-claim phrases (English + Korean whitelist), and exits 2 with stderr fed back as continuation context — forcing the model to invoke `verification-before-completion` and produce fresh evidence before Stop succeeds. **Spike evidence:** 2026-05-18 dry-run confirmed Claude Code honors `exit 2` from command-based Stop hooks (`docs/STATE.md` §"Command-hook Stop blocking spike (2026-05-18)"). **Companion-fix arc:** v0.11.3 input-layer parser + v0.11.4 `${CLAUDE_PLUGIN_ROOT}` path. **Known residuals (v0.11.8+):** LLM-class semantic similarity, speculative tense, multi-paragraph quote spans, Cherokee homoglyphs. **Portable invocation (v0.24.3):** the registered command is `sh "${CLAUDE_PLUGIN_ROOT}/scripts/hooks/run_hook.sh" "${CLAUDE_PLUGIN_ROOT}/scripts/hooks/stop_verify_claims.py"` — the shim resolves a WORKING Python ≥3.10 by functionality probe (`python3` → `python` → `py -3`; a PATH entry that exists but cannot run code, like the Windows Store App-Execution-Alias stub, is skipped) and `exec`s it so exit-2 blocking propagates unchanged; with no working interpreter it exits 1 (visible hook error + actionable stderr — a documented VISIBLE fail-open degrade, closing the 2026-07-01 silent fail-open incident; locked by `tests/test_regression_portable_hook_interpreter.py`). See `docs/archive/stop-hook-postmortem.md` + `docs/archive/defense-mechanisms-detail.md` for full pipeline, post-mortem, and detection-layer history.
...
**Single source of truth (v0.17.0 / S04 hoist).** Athanor's **Thin Leader** + **planner-classified discipline** applies across the 13 native Thin Leader skills (assess, analyze, debug, deep-plan, discuss, lfg, lfg-goal, lite-plan, plan, prompt-gen, review, setup, work). `superpowers:using-superpowers` is a Claude Code platform mechanism surfaced in SessionStart context, not Athanor hooks. Its mandatory tone is **advisory here**: discovery resolves through **leader dispatch**, not pre-response invocation. Carve-out: `scope-drift`, `verification-before-completion`, and `ce-test-browser`. **Honesty label: advisory**; no runtime gate ships. Regression lock: `tests/test_regression_v011_1_using_superpowers_boundary.py`. Each native skill carries a 2-line pointer instead of restating this prose. Concept adopted from superpowers v5.1.0 `sp-using-superpowers` (MIT, Jesse Vincent).
...
2. **Cross-model adversarial planning.** `/athanor:plan` via `--depth=` tier dispatch, with `/athanor:deep-plan` and `/athanor:lite-plan` as thin wrappers (deep = Planner A + Planner B Codex cross-planning + synthesis Critic; standard default = A + Codex review + refinement Critic; lite = A only).
...
- Learner / Codex-dispatcher: standard (sonnet) — note the `/athanor:lfg` Step 8.5 lfg-merge merges (+ deletes branch) only; the Releaser owns the version-bump/tag/CHANGELOG/STATE.md ceremony.
