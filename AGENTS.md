...
General-purpose agentic workflow orchestrator plugin for Codex. Current package-facing operator map: [docs/package-knowledge-index.md](docs/package-knowledge-index.md).
...
- In `/athanor:lfg` / `/athanor:lfg-loop`, under that pipeline's explicit user authorization, the leader may run git/gh plumbing (commit, push, open PR, CI watch) and read session/repo files for verification.
...
| `/athanor:discuss` | Plan | Decision brainstorming + intent clarification (dual mode: clarify ↔ synthesis). clarify = single-Codex gap-probe dialog → `requirements.md`; synthesis = Researcher + Devil's Advocate + Critic → `discuss.md` (v0.7.x behavior). |
...
| Read-Before-Edit Rule | **advisory** — prose guidance; Codex runtime is the practical enforcer for Codex-based workers, but no plugin-layer guard for Codex/non-Codex workers |
